                    ...
                    <div class="input-group input-group-sm" style="width: 150px;">
                        <div class="input-group-btn">
                            @if (Auth()->user()->access_level != '1')
                                <a class="btn btn-success pull-right" href="{{ url('usuario') }}"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Novo Cadastro</a>
                            @endif
                        </div>
                    </div>
                </div>
                                ...
                                <tbody>
                                    @foreach($usuarios as $usuario)
                                        <tr>
                                            <td>{{ $usuario->name }}</td>
                                            <td>{{ $usuario->email }}</td>
                                            <td>{{ Util::NivelUsuario($usuario->access_level) }}</td>
                                            <td>
                                                <center>
                                                    <table class="table-borderless">
                                                        <tr>
                                                            <td>
                                                                <a href="{{ url('usuario/'.$usuario->id) }}" title="Editar" class ="btn btn-sm btn-flat btn-info"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                                            </td>
                                                            <td>&nbsp;</td>
                                                            <td>
                                                                {!! Form::model($usuario, ['method' => 'DELETE', 'url' => ['usuario/' . $usuario->id], 'class' => 'form-inline form-delete']) !!}
...
<div class="modal modal-danger fade" id="modal_removeUsuario" style="display: none;">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="alert alert-dismissible">
                    <h4><i class="icon fa fa-ban"></i>ATENÇÃO! Tem certeza que deseja excluir este usuário?</h4>
...
<script>
    ...
    $('table[data-form="deleteForm"]').on('click', '.form-delete', function(e){
        e.preventDefault();
        var $form=$(this);
        $('#modal_removeUsuario').modal({ backdrop: 'static', keyboard: false })
